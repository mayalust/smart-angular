const log = require('proudsmart-log')(true),
  ...
  jsonloader = require("./json-loader.js"),
  {
    extend,
    getFilePath,
    isArray,
...
function inputPlugin(css) {
  return css ? [new angularLoaderPlugin(), new MiniCssExtractPlugin(css)] : [new angularLoaderPlugin()]
}
...
function pack() {
  ...
  psfile(pathLib.resolve(workpath, `./ps-${name}`)).children(n => {
    ...
    let time = new Date(),
      waitings = [...getConfig(), ...getCombinedConfig(), ...getSeparateConfig()];
